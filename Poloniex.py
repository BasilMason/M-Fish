...
class Poloniex:
    ...
    def api_query(self, command, req={}):
        ...
        if (command == "returnChartData"):
            return requests.get(
                ...
                + '&start=' + str(req['start']) + '&end=' + str(req['end']) + '&period=' + str(req['period'])).json()
            ...
            r = urllib.request.Request('https://poloniex.com/tradingApi', post_data, headers)
    ...
    # hacking it into the current strategy...
    def get_bids(self, pair, depth):
        return self.returnOrderBook(pair)['bids'][:depth]

    def get_asks(self, pair, depth):
        return self.returnOrderBook(pair)['asks'][:depth]
    ...
    def get_price(self, pair):
        ...
        while attempts > 0:
            ...
            try:
                return float(self.returnTicker()[pair]['last'])
